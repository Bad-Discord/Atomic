...
os.system('title ATOMIC - By M. logique')
...
def print_logo():
    ...
    banner = '''
        ...
        ██║  ██║   ██║   ╚█████╔╝██║ ╚═╝ ██║██║╚█████╔╝
        ╚═╝  ╚═╝   ╚═╝    ╚════╝ ╚═╝     ╚═╝╚═╝ ╚════╝'''
    b = Colorate.Vertical(Colors.DynamicMIX((Col.green, Col.yellow, Col.yellow)), Center.XCenter(banner))
...
Anime.Fade(Center.Center(intro), Colors.yellow_to_green, Colorate.Horizontal, interval=0.0035, enter=True)
...
async def tokenTools():
  ...
  1. Spam in one channel         8.  Webhook Spam Channels      15. Remove all friends           24. Check Token
  2. Spam in all channels        9.  Mass Create Roles          16. Block all friends            25. Check Tokens
  3. Add Reaction to a Message   10. Mass Create channels       17. Leave all servers            26. Get Guild info
  4. Join to a server            11. Delete all channels        18. Cycle Token Settings         27. Get Token Info
  5. Leave a server              12. Delete all roles           19. Mass dm                      28. Get Tokens Info
  6. Change Nickname             13. Remove all Emojis          20. Close all Dms                29. Back to Main Menu
  7. Change Status               14. Change server icon         21. Delete all personal guilds   30. Exit

    '''
    Write.Print(tokens_txt, Colors.yellow_to_green, interval=0.0005)
    print_('[~/Tokens] Choose an Option')
    ch = input_()
    if not ch in [str(i) for i in range(1,31)]:
        await tokenTools()
    elif ch == '29':
        await main_menu()
    elif ch == '30':
    ...
    if ch == '10':
        ...
        if len(tokens()) > 0 and count.isalnum() and type_.strip() in ['text', 'voice']:
            ...
            def create_channel(guild_id, name, type_):
                ...
                except:print_err(f"Failed to create {Col.red}{name}")
            for i in range(int(count)):
